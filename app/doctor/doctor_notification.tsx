...
export default function DoctorNotification() {
  ...
  const checkDueReminders = React.useCallback(async () => {
    try {
      ...
      const rems = Array.isArray(raw ? JSON.parse(raw) : []) ? JSON.parse(raw as string) : [];
      const now = Date.now();
      ...
      const cur = Array.isArray(rawN ? JSON.parse(rawN) : []) ? JSON.parse(rawN as string) : [];
      const toAdd = due.map((r: any) => {
        const title = r.kind === 'tomorrow' ? 'Appointment Reminder' : 'Appointment Starting Soon';
        const message = r.kind === 'tomorrow'
      ...
      await AsyncStorage.setItem('doctor_notifications', JSON.stringify([...toAdd, ...cur]));
        ...
        try {
          ...
          try {
            ...
            if (res.ok) {
              ...
              await AsyncStorage.setItem('doctor_notifications', JSON.stringify(merged));
        ...
        {showProfileMenu && (
          <View style={styles.dropdownOverlay}>
            ...
            <View style={[styles.dropdownCard, { top: insets.top + 48, right: 16 }]}> 
              <TouchableOpacity style={styles.dropdownItem} onPress={() => { setShowProfileMenu(false); navigation.navigate('DoctorProfile' as never); }}>
