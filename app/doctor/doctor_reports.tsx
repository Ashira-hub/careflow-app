...
export default function DoctorReports() {
    ...
    React.useCallback(() => {
      (async () => {
        ...
        try {
          ...
          const activityItem = { 
            id: String(Date.now()), 
            title: `Reports viewed for ${new Date(year, month).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}`, 
            type: 'reports', 
            timestamp: Date.now() 
          };
          await AsyncStorage.setItem('doctor_activity', JSON.stringify([activityItem, ...(Array.isArray(arrAct) ? arrAct.slice(0, 99) : [])]));
        } catch {}
        ...
        try {
          ...
          for (const r of Array.isArray(rows) ? rows : []) {
            ...
            if (!name) continue;
            if (!byPatient.has(name)) byPatient.set(name, { id: `PR-${name}`, name, appointments: [], prescriptions: [] });
            ...
            if (r.medicine || r.dosage) {
              rec.prescriptions.unshift({ doctorName: r.doctor || '', subject: r.medicine || '', quantity: '0', dosageStrength: r.dosage || '', description: r.notes || '', submittedAt: isNaN(ts) ? Date.now() : ts });
            } else {
              rec.appointments.unshift({ date: r.date || '', time: r.time || '', notes: r.notes || undefined, createdAt: isNaN(ts) ? Date.now() : ts });
            }
          }
          setRecords(Array.from(byPatient.values()));
        } catch {
          // Fallback to in-memory store
          try { setRecords(getRecords()); } catch { setRecords([]); }
        }
        try {
  ...
  const metrics = useMemo(() => {
    ...
    const perPatient = records.map((r) => {
      ...
      const lastTs = Math.max(appts[0]?.createdAt || 0, rxs[0]?.submittedAt || 0, ...appts.map(a=>a.createdAt), ...rxs.map(p=>p.submittedAt));
      const lastVisit = lastTs
        ? new Date(lastTs).toLocaleDateString(undefined, { month: 'short', day: '2-digit', year: 'numeric' })
        : '—';
      return { name: r.name, id: r.id, apptCount: appts.length, rxCount: rxs.length, lastVisit };
    });
    const totalAppointments = perPatient.reduce((acc, p) => acc + p.apptCount, 0);
    const totalPrescriptions = perPatient.reduce((acc, p) => acc + p.rxCount, 0);
    const totalPatients = perPatient.filter((p) => p.apptCount + p.rxCount > 0).length;
    const ranking = perPatient
      .map((p) => ({ name: p.name, id: p.id, lastVisit: p.lastVisit, count: p.apptCount + p.rxCount }))
      .filter((p) => p.count > 0)
      ...
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.cardsRow}>
            <SummaryCard label="Patients" value={metrics.totalPatients} tint="#D1FAE5" />
            <SummaryCard label="Appointments" value={metrics.totalAppointments} tint="#E0E7FF" />
          </View>
          <View style={styles.cardsRow}>
            <SummaryCard label="Prescriptions" value={metrics.totalPrescriptions} tint="#FEF3C7" />
          </View>
            ...
            {metrics.ranking.length === 0 ? (
              ...
              metrics.ranking.map((p) => (
                <View key={p.id} style={styles.row}> 
                  ...
                  <View style={styles.badge}><Text style={styles.badgeText}>{p.count}</Text></View>
                </View>
              ))
            )}
          </View>
