...
export default function LabEditProfile() {
  ...
  const roleLabel = (r?: string) => {
    ...
    if (key === 'labstaff' || key === 'lab' || key === 'laboratory') return 'Lab Staff';
  ...
  const onSave = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/users/${userId}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ name, email, role: roleValue || 'user', phone, address, birthdate, gender, avatar_uri: avatarUri }),
      });
      if (!res.ok) {
      ...
      try {
        ...
        const notif = {
          id: `LABPROF-UPD-${Date.now()}`,
          title: 'Profile Updated',
          message: 'Your laboratory profile has been updated successfully.',
          timestamp: Date.now(),
          read: false,
        } as any;
        await AsyncStorage.setItem('lab_notifications', JSON.stringify([notif, ...(Array.isArray(arrN) ? arrN : [])]));
      } catch {}
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.header, { paddingTop: insets.top }]}> 
          ...
          <View style={styles.headerIcons}>
            ...
            <TouchableOpacity style={styles.iconBtn} onPress={() => navigation.navigate('LabProfile' as never)}>
