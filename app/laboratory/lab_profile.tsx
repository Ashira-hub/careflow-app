...
export default function LabProfile() {
  ...
  useFocusEffect(React.useCallback(() => {
    (async () => {
      ...
      // Log recent activity: Viewed Profile
      try {
        const rawA = await AsyncStorage.getItem('lab_activity');
        const arrA = rawA ? JSON.parse(rawA) : [];
        const entry = { id: String(Date.now()), title: 'Viewed Profile', timestamp: Date.now(), type: 'profile' };
  ...
  const pickImage = () => {
    let pkg: any;
    try { pkg = require('react-native-image-picker'); } catch {}
    ...
    try {
      launchImageLibrary({ mediaType: 'photo', selectionLimit: 1 }, (res: any) => {
        ...
        if (res?.errorCode) { Alert.alert('Image Picker Error', String(res.errorMessage || res.errorCode)); return; }
            ...
            <View style={styles.profileCard}>
              ...
              <View style={{ flex: 1 }}>
                <View style={styles.nameRow}>
                  ...
                  <TouchableOpacity onPress={() => navigation.navigate('LabEditProfile' as never)} activeOpacity={0.85}>
