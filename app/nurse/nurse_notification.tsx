...
export default function NurseNotification() {
  ...
  const isSupervisorSchedule = (it: NotificationItem) => {
    return typeof it.title === 'string' && it.title.trim().toLowerCase().startsWith('new schedule:');
  };

  const parseScheduleFromNotif = (it: NotificationItem) => {
    // Title example: "New Schedule: Ward A" -> extract after colon
    const t = it.title || '';
    const colon = t.indexOf(':');
    const schedTitle = colon >= 0 ? t.slice(colon + 1).trim() : t.trim();
    // Message example from supervisor_schedule: "<Nurse> • <YYYY-MM-DD> • <Start> - <End> • <Note>"
    const parts = (it.message || '').split('•').map(s => s.trim()).filter(Boolean);
    // parts[0] is nurse name; parts[1] maybe date; parts[2] maybe start or start - end; trailing maybe note
    const date = parts[1] || '';
    ...
    return { title: schedTitle || 'Schedule', date, startTime, endTime, note };
  };

  const acceptNotification = async (it: NotificationItem) => {
    try {
      const raw = await AsyncStorage.getItem('nurse_notifications');
    ...
    // Add to nurse schedules storage
    try {
      const d = parseScheduleFromNotif(it);
      const sched = {
        id: `NS-${Date.now()}`,
        title: d.title,
  ...
  const renderItem = ({ item }: { item: NotificationItem }) => (
    <TouchableOpacity style={styles.card} activeOpacity={0.85} onPress={() => navigation.navigate('NurseNotifDetails' as never, { item } as never)}>
...
const styles = StyleSheet.create({
  ...
  acceptBtn: { paddingVertical: 6, paddingHorizontal: 10, borderWidth: 1, borderColor: GREEN, borderRadius: 8, backgroundColor: '#FFFFFF' },
