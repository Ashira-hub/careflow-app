...
export default function SupervisorEditProfile() {
  ...
  const onSave = async () => {
    try {
      ...
      const roleValue = role.toLowerCase() === 'supervisor' ? 'supervisor' : role.toLowerCase();
      const res = await fetch(`${API_BASE}/api/users/${userId}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ name, email, role: roleValue || 'supervisor', phone, address, birthdate, gender, avatar_uri: avatarUri || null }),
      });
      if (!res.ok) {
      ...
      // Also persist avatar uri under avatar_<userId> so headers load it
      try {
        if (avatarUri) {
      ...
      try {
        const rawAct = await AsyncStorage.getItem('supervisor_activity');
        ...
        await AsyncStorage.setItem('supervisor_activity', JSON.stringify([item, ...updatedAct]));
      } catch {}
      navigation.goBack();
    } catch (e: any) {
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.header, { paddingTop: insets.top }]}> 
          ...
          <View style={styles.headerIcons}>
            ...
            <TouchableOpacity style={styles.iconBtn} onPress={() => navigation.navigate('SupervisorProfile' as never)}>
        ...
        <View style={styles.bottomBar}>
          <BottomItem label="Home" source={require('../../assets/home_icon.png')} onPress={() => navigation.navigate('SupervisorDashboard' as never)} />
          <BottomItem label="Schedules" source={require('../../assets/appointment_icon.png')} onPress={() => navigation.navigate('SupervisorSchedule' as never)} />
